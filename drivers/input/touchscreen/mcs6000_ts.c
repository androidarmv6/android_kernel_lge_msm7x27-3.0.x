...
long mcs6000_ts_ioctl_down(struct file *file, unsigned int cmd, unsigned long arg)
{
	struct mcs6000_ts_data *ts = file->private_data;
	long err = 0;
...
static long mcs6000_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	long err = -1;
	...
	switch (_IOC_TYPE(cmd)) {
		case MCS6000_TS_DOWN_IOCTL_MAGIC:
			err = mcs6000_ts_ioctl_down(file, cmd, arg);
			break;
