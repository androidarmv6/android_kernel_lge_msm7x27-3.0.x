 ...
 *
 * $Id: dhd_cdc.c,v 1.22.4.2.4.7.2.12.4.35.2.12 2010/10/06 11:20:22 Exp $
 *
...
//#define CONFIG_LGE_BCM432X_PATCH_PMCTL
//#define CONFIG_LGE_BCM432X_PATCH_DTIMCTL
...
dhdcdc_cmplt(dhd_pub_t *dhd, uint32 id, uint32 len)
{
	...
	return dhd_bus_rxctl(dhd->bus, (uchar*)&prot->msg, len+sizeof(cdc_ioctl_t));
}
	...
	uint8 tim_bmp, uint8 mac_handle, uint32 htodtag)
{
	...
	eh = (struct ether_header *)PKTDATA(ctx->osh, p);
#endif
...
/* LGE_CHANGE_S, Disable setting power save mode if PM is 0 */		
		if (var_int == 0) {
			printk("[cdc]%s:%d - do not control PM (%d)\n",__func__,__LINE__,var_int);
			PM_control = FALSE;
		}
		else {
			printk("[cdc]%s:%d - do control PM (%d)\n",__func__,__LINE__,var_int);
			PM_control = TRUE;
		}
/* LGE_CHANGE_E, Disable setting power save mode if PM is 0 */
